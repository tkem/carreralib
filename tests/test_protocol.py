...
class ProtocolTest(unittest.TestCase):
    def test_chksum(self):
        for buf, res in (
            ...
            self.assertEqual(chksum(buf), res)
        for buf, offset, res in (
            (b"5321", 0, 0xB),
            (b":5321", 1, 0xB),
        ):
            self.assertEqual(chksum(buf, offset), res)
        for buf, offset, size, res in (
            (b"5321", 0, 4, 0xB),
            (b"5321:", 0, 4, 0xB),
            (b":5321", 1, 4, 0xB),
            (b":5321:", 1, 4, 0xB),
        ):
            self.assertEqual(chksum(buf, offset, size), res)
